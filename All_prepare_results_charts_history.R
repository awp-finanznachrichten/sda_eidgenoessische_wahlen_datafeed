...
#Adapt CVP/Mitte
results_parties_history_ch$partei_id <- as.numeric(ifelse(results_parties_history_ch$partei_id == 2,34,results_parties_history_ch$partei_id))
  
#Merge with parties metadata
results_parties_history_ch <- results_parties_history_ch %>%
  left_join(parties_metadata,
            by = join_by(partei_id == bfs_id))

#Merge with history data
parties_history <- results_parties_history_ch %>%
