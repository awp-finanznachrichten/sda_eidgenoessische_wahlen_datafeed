...
vorlage <- gsub("INSERT_NOTE",texts_intermediate_fr[9],vorlage)
...
vorlage <- gsub("INSERT_LOCATIONS",
                ...
                vorlage)
vorlage <- gsub("INSERT_CATCHWORD",texts_intermediate_fr[1],vorlage)
vorlage <- gsub("INSERT_HEADLINE",paste0("***TEST***",texts_intermediate_fr[2]),vorlage)
...
#Datei speichern
setwd("./Output")
filename <- paste0(format(Sys.Date(),"%Y%m%d"),"_NR_Intermediate_Wahlen2023_fr.xml")
