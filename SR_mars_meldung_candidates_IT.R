...
#Abschnitte kreieren
text <- paste0('<p class="paragraph">',texts_candidates_it[3],"</p>\n")
text <- paste0(text,tabelle_it,"\n")
text <- paste0(text,'<p class="paragraph">',texts_candidates_it[4],"</p>\n")
...
vorlage <- gsub("INSERT_NOTE",texts_candidates_it[5],vorlage)
...
vorlage <- gsub("INSERT_DATELINE",counted_cantons_SR$hauptort_it[c],vorlage)
...
vorlage <- gsub("INSERT_CHANNELS",
                paste0('<subject type="cpnat:abstract" qcode="sdamarschannel:ELE"></subject>\n',
                       ...
                       '<subject type="cpnat:abstract" qcode="sdamarschannel:UPH"></subject>'),
                vorlage)
vorlage <- gsub("INSERT_LOCATIONS",
                paste0('<located type="loctype:city" qcode="sdamarsgeo:',counted_cantons_SR$hauptort_mars_code[c],'">\n<name>',counted_cantons_SR$hauptort_it[c],'</name>\n</located>'),
                vorlage)
vorlage <- gsub("INSERT_CATCHWORD",texts_candidates_it[1],vorlage)
vorlage <- gsub("INSERT_HEADLINE",paste0("***TEST***",texts_candidates_it[2]),vorlage)
