...
#Abschnitte kreieren
text <- paste0('<p class="paragraph">',paste0(texts_parties[3]," ",texts_parties[4]),"</p>\n")
...
vorlage <- gsub("INSERT_DATELINE",counted_cantons$hauptort_de[c],vorlage)
...
vorlage <- gsub("INSERT_LOCATIONS",
                paste0('<located type="loctype:city" qcode="sdamarsgeo:',counted_cantons$hauptort_mars_code[c],'">\n<name>',counted_cantons$hauptort_de[c],'</name>\n</located>'),
                vorlage)
vorlage <- gsub("INSERT_CATCHWORD",texts_parties[1],vorlage)
vorlage <- gsub("INSERT_HEADLINE",paste0("***TEST***",texts_parties[2]),vorlage)
...
#Datei speichern
setwd("./Output_Mars")
filename <- paste0(format(Sys.Date(),"%Y%m%d"),"_",counted_cantons$area_ID[c],"_NR_Results_Wahlen2023_de.xml")
